...
int main(int argc, const char *argv[]) {
	...
	for (size_t i = 0; (i < filenames.size()) && (board_count < n_boards); ++i) {
		...
		bool found = cv::findChessboardCorners(image, board_sz, corners); //寻找棋盘内角点位置
		//其中image是输入的棋盘图
